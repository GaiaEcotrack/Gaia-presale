...
describe('GAIA ECOTRACK — 19-step REAL acceptance flow', () => {
  it('runs all 19 steps end-to-end', async () => {
    ...
    const auditSigs = new Set<string>()
    ...
    /* STEP 7 — foreign-program transaction -> WRONG_PROGRAM_ID */
    {
      ...
      const res = await purchase.POST(post('/x', { txSignature: VALID_SIG }))
      const body = ((await res.json()) as { error: string })
      expect(['WRONG_PROGRAM_ID', 'RPC_DATA_INCONSISTENT']).toContain(body.error)
      if (body.error === 'RPC_DATA_INCONSISTENT') expect(res.status).toBe(503)
    ...
    /* STEP 17 — canonical GET returns facts + protocol block + fresh flag */
    {
      ...
      // Totals accumulate across runs for this wallet (immutable history).
      expect(Number(json.data.summary.totalAcquiredGaia)).toBeGreaterThanOrEqual(25000)
    ...
    /* STEP 19 — SyncAttempt audit trail records rejections */
    {
      const attempts = await db.syncAttempt.findMany({
        where: { txSignature: { in: Array.from(auditSigs) } },
