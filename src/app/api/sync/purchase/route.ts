...
import { syncPurchaseTx } from '@/lib/server/db-sync'
...
// Canonical HTTP mapping for every verification rejection reason.
...
export async function POST(request: Request) {
  try {
    ...
    const schemaResult = purchaseSyncSchema.safeParse(parsedBody.body)
    ...
    const result = await syncPurchaseTx({
    ...
    return NextResponse.json({
      ...
      purchase: jsonSafe(result.purchase),
    })
  } catch (err) {
    console.error('[API] /api/sync/purchase error:', err)
